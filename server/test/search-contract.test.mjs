...
test("matches multi-word skills spread across a description", () => {
  const database = createSearchDatabase([
    { id: "spread", title: "Backend Engineer", description: "Experience with Python and JavaScript is required." },
    { id: "adjacent", title: "Data Engineer", description: "Python JavaScript both used daily." },
    { id: "one-only", title: "Support Specialist", description: "Python only, no front-end work." },
    { id: "split-phrase", title: "Tax Reviewer", description: "Personal tax planning including capital gains." },
  ]);

  // A description that reads "Python and JavaScript" must match a two-word
  // skill query; requiring one adjacent phrase silently dropped it.
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const result = searchDb(database, "Python JavaScript", null, 8);
    assert.equal(result.total, 2);
    assert.deepEqual(titles(result), ["Backend Engineer", "Data Engineer"]);
  }

  const separated = searchDb(database, "tax capital gains", null, 8);
  assert.equal(separated.total, 1);
  assert.deepEqual(titles(separated), ["Tax Reviewer"]);

  // Every token is still required, and tokens still match whole words only.
  assert.equal(searchDb(database, "Python Rust", null, 8).total, 0);
  assert.equal(searchDb(database, "Pyth", null, 8).total, 0);
  database.close();
});

test("drops work-mode qualifiers instead of requiring them as role tokens", () => {
  // The catalog is entirely remote and freelance, so these qualifiers carry
  // no filtering signal. Keeping them turned "remote AI trainer" into a
  // false zero-result response.
  const cases = [
    ["remote AI trainer", "AI trainer"],
    ["Remote AI Trainer", "AI Trainer"],
    ["work from home language evaluator", "language evaluator"],
    ["freelance Python opportunities", "Python"],
    ["remote coding jobs", "coding"],
    ["telecommute data analyst", "data analyst"],
    ["AI trainer", "AI trainer"],
    ["remote jobs", ""],
  ];

  for (const [input, expected] of cases) {
    assert.equal(parseSearch(input), expected, input);
  }
});
