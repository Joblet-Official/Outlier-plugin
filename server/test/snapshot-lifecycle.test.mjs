...
function createTemporaryDirectory() {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "outlier-snapshot-test-"));
...
async function startApp({ dbPath, snapshotDir, feedUrl, env = {} }) {
  ...
  const child = spawn(process.execPath, ["--import", "tsx", SERVER_ENTRY], {
    ...
    env: {
      ...
      OUTLIER_FEED_URL: feedUrl,
      OUTLIER_APPLY_HOST: APPLY_HOST,
      SYNC_INTERVAL_MS: "60000",
      FEED_FETCH_TIMEOUT_MS: "1500",
      OUTLIER_REFRESH_WORKER_TIMEOUT_MS: "5000",
      OUTLIER_STALE_AFTER_MS: "3600000",
      OUTLIER_MAX_STALE_MS: "86400000",
      OUTLIER_MIN_PROMOTED_JOBS: "25",
      ...env,
    },
...
async function readWidget(child) {
  return mcpRequest(child, "resources/read", { uri: "ui://outlier/job-cards-v6.html" });
}
...
test("rejects a small first snapshot below the absolute promotion floor", async () => {
  ...
  const app = await startApp({
    ...
    snapshotDir,
    feedUrl,
    env: { OUTLIER_MIN_PROMOTED_JOBS: "3000" },
  });
...
test("fails the entire refresh for an oversized job regardless of invalid-row allowance", async () => {
  ...
  const app = await startApp({
    ...
    snapshotDir,
    feedUrl,
    env: { OUTLIER_MAX_INVALID_JOB_PERCENT: "100" },
  });
...
test("enforces the scanned tool contract and distinct result states", async () => {
  ...
  const descriptor = tools.result.tools.find((tool) => tool.name === "search_outlier_opportunities");
  ...
  // The advertised maximum must be the maximum the server actually returns.
  assert.equal(descriptor.inputSchema.properties.limit.maximum, 8);
  ...
  // A limit above the advertised maximum is capped, never rejected: an
  // over-eager limit must not cost the user their search.
  for (const limit of [10, 500]) {
    const oversizedLimit = await callSearch(app, { query: "Fixture jobs", limit });
  ...
  // Every listing is remote, so a work-mode qualifier must be normalized out
  // of the role rather than becoming a token the listing has to contain.
  // Asserting zero here previously enshrined a false zero-result response.
  for (const query of ["remote AI", "work from home AI", "freelance AI", "remote AI jobs"]) {
    const remoteGermany = await callSearch(app, {
      query,
      market: { countryCode: "DE" },
      limit: 10,
    });
    assert.equal(remoteGermany.result.isError, undefined, query);
    assert.equal(remoteGermany.result.structuredContent.data.status, "ok", query);
    assert.equal(remoteGermany.result.structuredContent.data.totalResults, 1, query);
    assert.equal(remoteGermany.result.structuredContent.data.appliedFilters.query, "AI", query);
    assert.equal(remoteGermany.result.structuredContent.data.appliedFilters.limit, 8, query);
    assert.equal(
      remoteGermany.result.structuredContent.data.jobs[0].title,
      "AI Language Evaluator (German)",
      query,
    );
  }

  // Removing the qualifier must not turn a genuine miss into a broad match.
  ...
  assert.equal(noBroadRemoteRetry.result.structuredContent.data.appliedFilters.query, "product designer");
  assert.match(noBroadRemoteRetry.result.content[0].text, /do not retry/i);
  ...
  assert.equal(currentLocation.result.structuredContent.data.status, "ok");
  assert.equal(currentLocation.result.structuredContent.data.totalResults, 1);

  // The coarse hint is applied at region and country level only. A caller in
  // a neighbouring city of the same state must still see the listing rather
  // than a false zero result, and the city is never echoed back.
  const neighbouringCity = await callSearch(
    app,
    { query: "Statistics Expert", useCurrentLocation: true },
    { "openai/userLocation": { city: "Bellevue", region: "WA", country: "US" } },
  );
  assert.equal(neighbouringCity.result.structuredContent.data.status, "ok");
  assert.equal(neighbouringCity.result.structuredContent.data.totalResults, 1);
  assert.deepEqual(neighbouringCity.result.structuredContent.data.appliedFilters.location, {
    source: "currentLocation",
  });

  // A hint carrying no country or region is treated as unavailable.
  const cityOnlyHint = await callSearch(
    app,
    { query: "Statistics Expert", useCurrentLocation: true },
    { "openai/userLocation": { city: "Bellevue" } },
  );
  assert.equal(cityOnlyHint.result.isError, true);
  assert.equal(cityOnlyHint.result.structuredContent.data.status, "location_unavailable");
  ...
  const oversized = await fetch(`${app.baseUrl}/mcp`, {
    ...
    body: JSON.stringify({
      ...
      id: 99,
      method: "tools/call",
      params: { name: "search_outlier_opportunities", arguments: { query: "x".repeat(70_000) } },
    }),
  });
...
test("rate-limits abusive MCP clients without changing ordinary requests", async () => {
  ...
  const app = await startApp({
    ...
    env: {
      SYNC_INTERVAL_MS: "86400000",
      OUTLIER_MCP_RATE_LIMIT_MAX_REQUESTS: "1",
    },
  });

  const first = await mcpRequest(app, "tools/list", {});
  assert.ok(first.result.tools.some((tool) => tool.name === "search_outlier_opportunities"));
